...
tas_validate_text() { LC_ALL=C awk 'BEGIN {v=ARGV[1]; if (v ~ /[\001-\011\013-\037\177]/) exit 1}' "${1-}"; }
...
tas_signal_group() { lock=$(tas_lock_name "${1-}"); tas_tmux wait-for -S "$lock"; }
