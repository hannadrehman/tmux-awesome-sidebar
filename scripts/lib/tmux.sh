...
tas_list_content_panes() { window_id=$1; tas_validate_id "$window_id" window || return 2; tas_tmux list-panes -t "$window_id" -F '#{pane_id}\t#{@awesome_sidebar_kind}' | awk -F '\t' '$2!="sidebar" {print $1}'; }
