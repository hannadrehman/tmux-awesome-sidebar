...
tas_list_group_windows() { group=${1-}; tas_validate_text "$group" || return 2; tab=$(printf '\t'); tas_tmux list-windows -a -F "#{window_id}${tab}#{session_id}${tab}#{@awesome_sidebar_group}${tab}#{@awesome_sidebar_kind}${tab}#{@awesome_sidebar_order}${tab}#{window_name}" | awk -F '\t' -v g="$group" '$3==g && $4!="sidebar" && !seen[$1]++ {print}' | sort -t '	' -k5,5n -k1,1n; }
